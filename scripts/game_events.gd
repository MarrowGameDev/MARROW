...
signal player_health_changed(player: Node, hp: int, max_hp: int)
signal player_damaged(player: Node, amount: int, source: Node)
# The player's detached body being hijacked / released is a multi-system event (enemies, player, UI).
signal body_possessed(body: Node, by: Node)
signal body_freed(body: Node)
